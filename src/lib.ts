export function split(
  target: string | any[],
  step: number,
  markString: boolean = typeof target === 'string'
) {
...
export function to_binary(
  code: number,
  bit: number = 8,
  max: number = Math.pow(2, bit) - 1
) {
...
export function utf16_to_utf8(str: string) {
  return str
    .split('')
    .map((char: string) => utf8_encode(char))
    .join('');
}
